...
void serverDeSerialize(void* buffer,
  ...
  printf("Got the number of params: ",nparams);
    ...
    int j;
  	for(int j=0; j<nparams; j++)
  	{
  		if(j==0)
  		{
  			...
  			argList->next = NULL;
  		  printf("ArgumentSize: %d",argList->arg_size );
  			printf("Argument val: %d",*((int*)(argList->arg_val)) );
  			prevArg = argList;
  		...
  		else
  		{
  			...
  			curArg->next = NULL;
  			printf("ArgumentSize: %d",curArg->arg_size);
  			printf("Argument val: %d",*((int*)(curArg->arg_val)) );
  			prevArg->next = curArg;
...
{
	int baseoffset = 0;
	printf("Value in the input arg %d",*((int*)(return_val)+2) );
	...
	printf("Size value: %d",((int*)returnBuf)[0]);
...
/////////////////////////////////////////////////////////////////////////////
    ...
    if (getsockname(server_sock, (struct sockaddr *) &my_addr, &my_addr_len) != -1)
    {
        ...
        {
            printf("%s %s",host_ip_rpc,htons(server_addr.sin_port));
...
///////////////////////////////////////////////////////////////////////////////
    ...
    while(1){
        ...
        if(count==-1) cout<<"RECV ERROR\n";else {
            ...
            printf("Current return value size: %d",current_rt.return_size);
            printf("Num Args: %d",num_of_args);
            current_rt = requested_fn.fn_pointer( num_of_args, &all_args );

            printf("Current return value size: %d",current_rt.return_size);
            void * new_val = malloc(current_rt.return_size);
	          printf("Function Exited giving %d",*((int *)(current_rt.return_val)+2) );
